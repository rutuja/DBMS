...
{
    // Functions tested on Large #records
    // 1. Create Index
    // 2. OpenIndex
    // 3. Insert entry
    // 4. Scan entries NO_OP -- open**
	// 5. DeleteEntry
	// 6. Scan after deletion
    // 7. Scan close **
    // 8. CloseIndex
    // 8. DestroyIndex
    // NOTE: "**" signifies the new functions being tested in this test case.
...
//    unsigned maxValue = 500;
    // Test InsertEntry : Inserting entries in Ascending Order
    for(unsigned i = 1; i <= numOfTuples; i++)
    {
        unsigned key = i+1;//just in case somebody starts pageNum and recordId from 1
        RID rid;
...
//    }
    ...
    //DeleteEntry: deleting entries in descending order
    for(unsigned i = numOfTuples; i > 0 ; i--)
    {
           ...
           if(rc != success)
           {
               cout << "Failed Deleting Entry..." << key << endl;
           }
           else
		   {
		   	rc = ixScan->OpenScan(ixHandle, EQ_OP, &key);
    ...
    // Test InsertEntry : Inserting entries in Descending Order
    for(unsigned i = numOfTuples; i > 0; i--)
    {
        unsigned key = i+1;//just in case somebody starts pageNum and recordId from 1
        RID rid;
...
//    }
    ...
    //DeleteEntry: deleting entries in ascending order
    rc = ixScan->CloseScan();
    for(unsigned i = 1; i <= numOfTuples; i++)
    {
           unsigned key = i+1;//just in case somebody starts pageNum and recordId from 1
...
{
	//Repeating testCase_10 for TypeReal
    // Functions tested on Large #records
    // 1. Create Index
    // 2. OpenIndex
    // 3. Insert entry
...
//    unsigned maxValue = 500;
    ...
    for(unsigned i = 1; i <= numOfTuples; i++)
    {
        float key = i+1.5;//just in case somebody starts pageNum and recordId from 1
        RID rid;
        rid.pageNum = i;
        rid.slotNum = i+1;
...
//Delete Duplicate
    for(unsigned i = 0; i < numOfTuples; i++)
    {
    	...
    	if(rc != success)
    	{
    		cout << "Failed Deleting Keys..." << endl;
    	}
    }
...
void testCase_11(const string tablename, const string attrname)
{
    // Functions tested
    // 1. Create Index
    // 2. OpenIndex
    // 3. Insert entry
    // 4. Scan entries, and delete some entries
    // 5. Scan entried again
    // 6. scan all options
    // 7. Scan close
    // 8. CloseIndex
    // 9. DestroyIndex
   ...
   // Test Scan
    ...
    while(ixScan->GetNextEntry(rid) == success)
    {
        ...
        if(rid.pageNum%5==0|| rid.slotNum%5==0)
        {
            cout << "Wrong entries output...failure" << endl;
        }
    }
...
//    // Duplicate Entries
    ...
    testCase_11("tbl_employee", "Height");
    //Duplicate Entries
    testCase_extra_3("tbl_employee", "Age");
    return 0;
}
*/
